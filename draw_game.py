...
class GameRenderer:
    def __init__(self, screen, WIDTH, HEIGHT, player_width, player_height, 
        ...
        try:
            ...
            explosion_gif = pygame.image.load('assets/weapons/explosion.gif')
    ...
    def draw_background(self, env_name):
        """Draw the environment background"""
        # Check if we have a loaded background for this environment
        if env_name in self.backgrounds:
            # Draw the cached background
            self.screen.blit(self.backgrounds[env_name], (0, 0))
        else:
            # Fallback background
    ...
    def draw_debug_info(self, player_x, player_y, font, color=(255, 255, 255)):
        """Draw debug information"""
        debug_text = font.render(f"Player Pos: ({player_x}, {player_y})", True, color)
        self.screen.blit(debug_text, (10, 100))
    ...
    def draw_stat_upgrade_menu(self, stats, current_selection, font_medium, font_small, upgrade_points):
        ...
        for i, (stat, value) in enumerate(stats.items()):
            ...
            stat_text = font_small.render(f"{formatted_stat}: {value:.2f}", True, color)
            self.screen.blit(stat_text, (x_center - stat_text.get_width() // 2, y_start + i * y_spacing))
        
        # Instructions
        instructions_y = y_start + len(stats) * y_spacing + 30
        
        up_text = font_small.render("Up/Down: Select Stat", True, (150, 150, 150))
        self.screen.blit(up_text, ((self.WIDTH - up_text.get_width()) // 2, instructions_y))
        
        enter_text = font_small.render("Enter/Space: Upgrade Selected Stat", True, (150, 150, 150))
        self.screen.blit(enter_text, ((self.WIDTH - enter_text.get_width()) // 2, instructions_y + 30))
        
        esc_text = font_small.render("ESC: Return to Game", True, (150, 150, 150))
        self.screen.blit(esc_text, ((self.WIDTH - esc_text.get_width()) // 2, instructions_y + 60))
    ...
    def draw_map_object(self, obj):
        ...
        if obj.type == 'door':
            ...
            try:
                door_img = pygame.image.load('assets/general/door.jpg')
