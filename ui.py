...
class GameUI:
    ...
    def draw_inventory(self, screen, inventory_system):
        """Draw the inventory UI (placeholder implementation)"""
        if not self.show_inventory:
            return
            
        # Simple inventory background
        panel_width = 400
        panel_height = 300
        panel_x = (self.WIDTH - panel_width) // 2
        panel_y = (self.HEIGHT - panel_height) // 2
        ...
        for i, slot in enumerate(inventory_system.slots):
            if i >= 20:  # Draw maximum 20 slots
                break
            ...
            pygame.draw.rect(screen, DARK_GRAY, (slot_x, slot_y, slot_size, slot_size))
            
            # If there's an item in this slot
            if slot.item:
                # Draw a placeholder for the item
                pygame.draw.rect(screen, (100, 100, 100), (slot_x + 5, slot_y + 5, slot_size - 10, slot_size - 10))
                
                # If it has quantity
                if slot.quantity > 1:
                    qty_text = self.small_font.render(str(slot.quantity), True, WHITE)
                    screen.blit(qty_text, (slot_x + slot_size - qty_text.get_width() - 5, slot_y + slot_size - qty_text.get_height() - 5))
            
            # Draw slot border
            border_color = YELLOW if i == self.inventory_selected_slot else WHITE
    ...
    def handle_inventory_input(self, event, inventory_system):
        """Handle input events for the inventory UI"""
        if not self.show_inventory:
            return False
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_UP:
                self.inventory_selected_slot = max(0, self.inventory_selected_slot - 5)
                return True
            elif event.key == pygame.K_DOWN:
                self.inventory_selected_slot = min(19, self.inventory_selected_slot + 5)
                return True
            elif event.key == pygame.K_LEFT:
                if self.inventory_selected_slot % 5 > 0:
                    self.inventory_selected_slot -= 1
                return True
            elif event.key == pygame.K_RIGHT:
                if self.inventory_selected_slot % 5 < 4 and self.inventory_selected_slot < 19:
                    self.inventory_selected_slot += 1
                return True
                ...
                # Use selected item
                if self.inventory_selected_slot < len(inventory_system.slots):
                    inventory_system.use_item(self.inventory_selected_slot)
                return True
                
        return False 

    def draw_map(self, screen, current_environment, world_map=None):
        """Draw the map UI when open"""
        if not self.show_map:
            return
            
        # Create semi-transparent overlay
        overlay = pygame.Surface((self.WIDTH, self.HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 200))  # 80% opacity black background
        screen.blit(overlay, (0, 0))
        
        # Draw map title
        title = self.font.render("MAP", True, WHITE)
        screen.blit(title, (self.WIDTH // 2 - title.get_width() // 2, 50))
        
        # Draw a larger version of the minimap
        map_width = self.WIDTH * 0.7
        map_height = self.HEIGHT * 0.7
        map_x = (self.WIDTH - map_width) // 2
        map_y = (self.HEIGHT - map_height) // 2 + 20
        
        # Draw map background
        pygame.draw.rect(screen, (30, 30, 30), (map_x, map_y, map_width, map_height))
        pygame.draw.rect(screen, (150, 150, 150), (map_x, map_y, map_width, map_height), 2)
        
        # If no world map was provided, use fixed implementation
        if world_map is None:
            self._draw_map_fixed(screen, map_x, map_y, map_width, map_height, current_environment)
            return
            
        # Use world map to draw the map dynamically
        # Calculate coordinate multipliers
        section_width = map_width / 7  # 7 horizontal positions
        section_height = map_height / 3  # 3 vertical positions
        
        # Define node size
        node_size = 30  # Bigger nodes for the full map view
        
        # Draw connections first (so they appear under the nodes)
        for env_name, connected_envs in world_map.connections.items():
            # Get the position of this environment
            source_pos = world_map.get_position(env_name)
            source_x = map_x + section_width * (source_pos[0] + 0.5)
            source_y = map_y + section_height * (source_pos[1] + 0.5)
            
            # Draw connections to each connected environment
            for target_env in connected_envs:
                target_pos = world_map.get_position(target_env)
                target_x = map_x + section_width * (target_pos[0] + 0.5)
                target_y = map_y + section_height * (target_pos[1] + 0.5)
                
                # Draw the connection line
                pygame.draw.line(screen, WHITE, (source_x, source_y), (target_x, target_y), 3)
        
        # Draw all environment nodes
        font = pygame.font.SysFont(None, 24)  # Bigger font for full map
        for env_name, pos in world_map.map_positions.items():
            # Calculate pixel position
            x = map_x + section_width * (pos[0] + 0.5)
            y = map_y + section_height * (pos[1] + 0.5)
            
            # Choose color based on whether this is the current environment
            color = world_map.get_color(env_name, is_active=(env_name == current_environment))
            ...
            # Position labels based on vertical position
            if pos[1] == 0:  # Top row
                screen.blit(label, (x - label.get_width()//2, y - node_size - label.get_height() - 5))
            elif pos[1] == 2:  # Bottom row
                screen.blit(label, (x - label.get_width()//2, y + node_size + 5))
            else:  # Middle row
                screen.blit(label, (x - label.get_width()//2, y + node_size//2 + 5))
        
        # Draw instructions at the bottom
        instructions = self.small_font.render("Press ESC or M to close map", True, WHITE)
        screen.blit(instructions, (self.WIDTH // 2 - instructions.get_width() // 2, 
                                self.HEIGHT - 50))
    
    def _draw_map_fixed(self, screen, map_x, map_y, map_width, map_height, current_env):
        """Draw a fixed map layout when no world map object is available"""
        # Calculate node positions based on the map flow
        # City > Apartments > Starting > Streets > Forest > Lake > Swamp
        
        section_width = map_width / 7  # Divide available width
        node_size = 30  # Bigger nodes for the full map view
        
        # Main horizontal path nodes
        city_x = map_x + section_width * 0.5
        city_y = map_y + map_height // 2
        
        apartment_x = map_x + section_width * 1.5
        apartment_y = map_y + map_height // 2
        
        start_x = map_x + section_width * 2.5
        start_y = map_y + map_height // 2
        
        streets_x = map_x + section_width * 3.5
        streets_y = map_y + map_height // 2
        
        forest_x = map_x + section_width * 4.5
        forest_y = map_y + map_height // 2
        
        lake_x = map_x + section_width * 5.5
        lake_y = map_y + map_height // 2
        
        swamp_x = map_x + section_width * 6.5
        swamp_y = map_y + map_height // 2
        
        # Vertical path nodes
        rooftop_x = apartment_x
        rooftop_y = map_y + map_height // 5
        
        room_x = start_x
        room_y = map_y + map_height // 5
        
        sewer_x = streets_x
        sewer_y = map_y + map_height * 4 // 5
        
        # Draw horizontal connections (main path)
        pygame.draw.line(screen, WHITE, (city_x + node_size//2, city_y), (apartment_x - node_size//2, apartment_y), 3)
        pygame.draw.line(screen, WHITE, (apartment_x + node_size//2, apartment_y), (start_x - node_size//2, start_y), 3)
        pygame.draw.line(screen, WHITE, (start_x + node_size//2, start_y), (streets_x - node_size//2, streets_y), 3)
        pygame.draw.line(screen, WHITE, (streets_x + node_size//2, streets_y), (forest_x - node_size//2, forest_y), 3)
        pygame.draw.line(screen, WHITE, (forest_x + node_size//2, forest_y), (lake_x - node_size//2, lake_y), 3)
        pygame.draw.line(screen, WHITE, (lake_x + node_size//2, lake_y), (swamp_x - node_size//2, swamp_y), 3)
        
        # Draw vertical connections
        pygame.draw.line(screen, WHITE, (apartment_x, apartment_y - node_size//2), (rooftop_x, rooftop_y + node_size//2), 3)
        pygame.draw.line(screen, WHITE, (start_x, start_y - node_size//2), (room_x, room_y + node_size//2), 3)
        pygame.draw.line(screen, WHITE, (streets_x, streets_y + node_size//2), (sewer_x, sewer_y - node_size//2), 3)
        ...
        # Add labels with a larger font for the full map
        ...
        # Draw all labels below their nodes except for vertical ones
        screen.blit(city_label, (city_x - city_label.get_width()//2, city_y + node_size//2 + 10))
        screen.blit(apt_label, (apartment_x - apt_label.get_width()//2, apartment_y + node_size//2 + 10))
        screen.blit(start_label, (start_x - start_label.get_width()//2, start_y + node_size//2 + 10))
        screen.blit(streets_label, (streets_x - streets_label.get_width()//2, streets_y + node_size//2 + 10))
        screen.blit(forest_label, (forest_x - forest_label.get_width()//2, forest_y + node_size//2 + 10))
        screen.blit(lake_label, (lake_x - lake_label.get_width()//2, lake_y + node_size//2 + 10))
        screen.blit(swamp_label, (swamp_x - swamp_label.get_width()//2, swamp_y + node_size//2 + 10))
        
        # Draw vertical connection labels to the side
        screen.blit(room_label, (room_x - room_label.get_width() - 10, room_y - room_label.get_height()//2))
        screen.blit(roof_label, (rooftop_x - roof_label.get_width() - 10, rooftop_y - roof_label.get_height()//2))
        screen.blit(sewer_label, (sewer_x + node_size//2 + 10, sewer_y - sewer_label.get_height()//2)) 